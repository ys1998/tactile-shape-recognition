...
def receive() :
    ...
    while True :
        ...
        if waiting >= 4 :
            ...
            if endByte < 3 :
                continue
            if rawQueue[endByte-3] == 1 :
                recvQueue = np.zeros(64)
                for row in range(4) :
                    for col in range(4) :
                        dataQueue[0][col][row] = rawQueue[endByte-2]*256+rawQueue[endByte-1]
                update = 1
                print("Received packet : ",recvQueue)
        ...
        while True :
            ...
            if update == 1 : 
                ...
                for pos in range(64) :
                    ...
                    row = (pos%16)%4
                    if patchNum == 0 :
                        self.intensityData[0][row][col][0] = max(0,2*int(dataQueue[patchNum][row][col]/256)-255)
                        self.intensityData[0][row][col][2] = max(0,255-2*int(dataQueue[patchNum][row][col]/256))
                        ...
                        self.intensityData[1][row][col][0] = max(0,2*int(dataQueue[patchNum][row][col]/256)-255)
                        self.intensityData[1][row][col][2] = max(0,255-2*int(dataQueue[patchNum][row][col]/256))
                        ...
                        self.intensityData[2][row][col][0] = max(0,2*int(dataQueue[patchNum][row][col]/256)-255)
                        self.intensityData[2][row][col][2] = max(0,255-2*int(dataQueue[patchNum][row][col]/256))
                        ...
                        self.intensityData[3][row][col][0] = max(0,2*int(dataQueue[patchNum][row][col]/256)-255)
                        self.intensityData[3][row][col][2] = max(0,255-2*int(dataQueue[patchNum][row][col]/256))
